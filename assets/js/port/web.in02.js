...
// What a reader is SHOWN for a shelf code (MOFaithLabel, lib/faith-catalogue.js): "English Divines" prints as "English writers". SHN itself stays the source vocabulary.
const SHD=k=>{const l=SHN[k];return l&&window.MOFaithLabel?window.MOFaithLabel.shelf(l):l;};
...
async function boot(){
  ...
  if(shsel)Object.entries(SHN).forEach(([k,l])=>{const o=document.createElement("option");
    o.value=k;o.textContent=SHD(k)||l;shsel.appendChild(o);});
...
const saveBtn=(w,p,title,author,label)=>{if(!w||p==null||p==='')return '';const on=SAVED.has(refKey(w,p));return `<button type="button" class="pinb${on?' on':''}" data-feature-gate="tfr-notebook" data-save-passage="${esc(w)}" data-page="${esc(String(p))}" data-title="${esc(String(title||'').slice(0,90))}" data-author="${esc(String(author||'').slice(0,60))}" data-label="${esc(String(label||'').slice(0,240))}" aria-pressed="${on}">${on?'Saved':'Save passage'}</button>`;};
const saveWorkBtn=(w,title,author)=>{if(!w)return '';const on=SAVED.has(refKey(w,null));return `<button type="button" class="pinb pinb-work${on?' on':''}" data-feature-gate="tfr-notebook" data-save-work="${esc(w)}" data-title="${esc(String(title||'').slice(0,120))}" data-author="${esc(String(author||'').slice(0,60))}" aria-pressed="${on}">${on?'Saved':'Save work'}</button>`;};
...
(function(){
  ...
  addEventListener("mousemove",e=>{
    ...
    if(n){tip.style.display="block";
      ...
      tip.innerHTML=`<b>${esc(n.a)}</b><span>${n.yok?"c. "+n.y+" · ":""}${esc(ERAL[n.e]||"")}${n.sh?" · "+esc(SHD(n.sh)||n.sh):""}</span><br><span>PageRank ${n.pr.toFixed(1)} · cited by ${n.din} · community: ${esc(n.com)}</span>`;
...
async function openAuthor(slug,push=true){
  ...
  const __my=++__PSEQ;
  if(networkLayout!=="timeline")window.MOFaithConnections.showAuthor(slug);
  ...
  openPanel(esc(n.a),`${n.yok?"c. "+n.y+" · ":""}${esc(ERAL[n.e]||"")}${n.sh?" · "+esc(SHD(n.sh)||""):""}${ctr}`);
...
function journeyURL(chain,direction='in',work='',targetWork='',reference='',via=[]){return '/the-faith-received/connections/#journey='+chain.map(encodeURIComponent).join(',')+'?'+new URLSearchParams({direction,...work?{work}:{},...targetWork?{targetWork}:{},...reference?{reference}:{},...(direction==='either'&&via.length)?{via:via.join(',')}:{}});}
...
function networkHash(layout,author){const view=window.MOFaithConnections.state().view;return "#shelves=citations/"+(view==="contested"?view:"cited")+"?"+new URLSearchParams({arrange:layout,...(author?{author}: {})});}
...
function openShelfMaps(shelf='citations',kind='cited',push=true,options={}){
 ...
 window.MOFaithConnections.open({shelf,view:kind,arrange:options.arrange||'',entry:options.entry||'',query:options.query||'',author:options.author||''});
