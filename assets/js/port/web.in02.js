...
function journeyURL(chain,direction='in',work='',targetWork='',reference='',via=[]){return '/the-faith-received/constellations/#journey='+chain.map(encodeURIComponent).join(',')+'?'+new URLSearchParams({direction,...work?{work}:{},...targetWork?{targetWork}:{},...reference?{reference}:{},...(direction==='either'&&via.length)?{via:via.join(',')}:{}});}
