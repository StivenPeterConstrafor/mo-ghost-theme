...
(function(root){
  ...
  const WITNESS_FAMILIES={
    'quenstedt-theologia-didactico-polemica':{work:'quenstedt-theologia',label:'Witness 1 · page scans'},
    'quenstedt-systema-theologicum':{work:'quenstedt-theologia',label:'Witness 2 · Wittenberg, 1685'}
  };
  function witnessSummary(rows){
    const witnesses=[...new Set(rows.map(r=>String(r.w||'')))];
    const families=new Map();for(const w of witnesses){const key=WITNESS_FAMILIES[w]?.work||w;if(!families.has(key))families.set(key,[]);families.get(key).push(w);}
    return {records:rows.length,works:families.size,witnesses:witnesses.length,hasAlternates:[...families.values()].some(a=>a.length>1)};
  }
  ...
  function citationViewState(value,scope){
    ...
    return {scope:scope.slice(),filters,workPage:page(src.workPage),recordPages:Object.fromEntries(Object.entries(src.recordPages||{}).map(([w,n])=>[w,page(n)])),recordSizes:Object.fromEntries(Object.entries(src.recordSizes||{}).map(([w,n])=>[w,Math.max(1,page(n))])),openFolds:Array.isArray(src.openFolds)?src.openFolds.filter(x=>typeof x==='string'):null,filterOpen:!!src.filterOpen,previews:Array.isArray(src.previews)?src.previews.filter(x=>safeURL(x)):[]};
  }
  ...
  function mountCitation(host,options){
    ...
    const kinds=[...new Set(rows.map(r=>r.h).filter(Boolean))].sort(),targets=new Map(),editorialCount=rows.filter(r=>sourceContext(r.w,r.p)?.layer==='editorial-notes').length;let workPage=resume.workPage;const recordPages=new Map(Object.entries(resume.recordPages));const recordSizes=new Map(Object.entries(resume.recordSizes||{}));if(resume.openFolds)host.__ceOpen=new Set(resume.openFolds);
    ...
    function capture(){return citationViewState({scope:[citing.s,target.s],filters:Object.fromEntries(['work','kind','context','query','target','layer'].map(k=>[k,find('[data-ce-'+k+']')?.value||''])),workPage,recordPages:Object.fromEntries(recordPages),recordSizes:Object.fromEntries(recordSizes),openFolds:host.__ceOpen?[...host.__ceOpen]:null,filterOpen:find('.ce-filter-options')?.open,previews:[...host.querySelectorAll('[data-ce-preview][open]')].map(el=>el.dataset.cePreview)},[citing.s,target.s]);}
    ...
    function paint(){
      ...
      const coverage=witnessSummary(hits);find('.ce-count').textContent=coverage.hasAlternates?count(coverage.works)+' work'+(coverage.works===1?'':'s')+' in '+count(coverage.witnesses)+' witnesses. Counts are shown separately below; overlapping citations have not been deduplicated.':count(hits.length)+(hits.length===1?' reference record':' reference records')+(+entry.n>rows.length?' · '+count(entry.n)+' counted in the index':'');
      ...
      const wire=frag=>{frag.querySelectorAll('[data-ce-pick]').forEach(input=>input.onchange=()=>{const r=rows[+input.dataset.cePick],s=source(r,works,citing.a);if(input.checked)selected.set(s.url,s);else selected.delete(s.url);box.querySelectorAll('[data-ce-pick]').forEach(el=>{el.checked=selected.has(source(rows[+el.dataset.cePick],works,citing.a).url);});updateSelection();});bindPreviews(frag);bindPageContexts(frag);
        frag.querySelectorAll('[data-ce-fold]').forEach(d=>d.addEventListener('toggle',()=>{if(!d.isConnected)return;if(d.open)openFolds.add(d.dataset.ceFold);else openFolds.delete(d.dataset.ceFold);}));};
      ...
      const saveBtn=(w,p,title,label)=>{if(!N?.savePassage||!w||p==null||p==='')return '';const on=savedKeys.has(rk(w,p));return `<button type="button" class="pinb${on?' on':''}" data-feature-gate="tfr-notebook" data-save-passage="${esc(w)}" data-page="${esc(String(p))}" data-title="${esc(String(title||'').slice(0,90))}" data-author="${esc(String(citing.a||'').slice(0,60))}" data-label="${esc(String(label||'').slice(0,240))}" aria-pressed="${on}">${on?'Saved':'Save passage'}</button>`;};
      const saveWorkBtn=(w,title)=>{if(!N?.saveWork||!w)return '';const on=savedKeys.has(rk(w,null));return `<button type="button" class="pinb pinb-work${on?' on':''}" data-feature-gate="tfr-notebook" data-save-work="${esc(w)}" data-title="${esc(String(title||'').slice(0,120))}" data-author="${esc(String(citing.a||'').slice(0,60))}" aria-pressed="${on}">${on?'Saved':'Save work'}</button>`;};
      ...
      function recordPage(w,rs){const size=recordSizes.get(w)||25,p=pageWindow(rs,recordPages.get(w)||0,size);recordPages.set(w,p.page);return `<div class="ce-record-options"><p>${p.start+1}–${p.end} of ${count(rs.length)} records</p>${rs.length>25?`<button type="button" data-ce-show-all="${esc(w)}">${size>=rs.length?'Show 25 at a time':'Show all '+count(rs.length)+' records'}</button>`:''}</div>`+pager(p,'records',w)+p.items.map(recordHTML).join('')+pager(p,'records',w);}
      box.innerHTML=pager(win,'works')+win.items.map(([w,rs])=>{const s=source(rs[0],works,citing.a);return `<details class="ce-work" data-ce-fold="${esc(w)}"${openFolds.has(w)?' open':''}><summary><span><strong>${esc(s.title)}</strong></span><small class="ce-n">${count(rs.length)} records${WITNESS_FAMILIES[w]?' · '+esc(WITNESS_FAMILIES[w].label):''} · View references</small></summary><div class="ce-work-actions"><a class="ce-open-work" href="${esc(readerURL(w))}" target="_blank" rel="noopener">Open work</a>${saveWorkBtn(w,s.title)}</div><div class="ce-fold-body" data-ce-record-page="${esc(w)}">${recordPage(w,rs)}</div></details>`;}).join('')+pager(win,'works');
      wire(box);
      function wirePages(parent){parent.querySelectorAll('button[data-ce-page]').forEach(button=>button.onclick=()=>{const kind=button.dataset.cePage,w=button.dataset.cePageWork,page=Number(button.dataset.page);if(kind==='works'){workPage=page;paint();return;}const body=box.querySelector('[data-ce-record-page="'+CSS.escape(w)+'"]'),rs=groups.find(g=>g[0]===w)?.[1]||[];if(!body)return;recordPages.set(w,page);body.innerHTML=recordPage(w,rs);wire(body);wirePages(body);wireAll(body);const focus=body.querySelector('[data-page="'+(page+1)+'"]:not(:disabled)')||body.querySelector('[data-ce-page]:not(:disabled)');focus?.focus({preventScroll:true});body.scrollIntoView({block:'start',behavior:'auto'});});}
      function wireAll(parent){parent.querySelectorAll('[data-ce-show-all]').forEach(button=>button.onclick=()=>{const w=button.dataset.ceShowAll,rs=groups.find(g=>g[0]===w)?.[1]||[],body=box.querySelector('[data-ce-record-page="'+CSS.escape(w)+'"]');if(!body)return;recordSizes.set(w,(recordSizes.get(w)||25)>=rs.length?25:rs.length);recordPages.set(w,0);body.innerHTML=recordPage(w,rs);wire(body);wirePages(body);wireAll(body);});}
      find('.ce-expand-all').onclick=()=>{groups.forEach(([w])=>openFolds.add(w));box.querySelectorAll('details.ce-work').forEach(d=>{d.open=true;openFolds.add(d.dataset.ceFold);});};
      wireAll(box);wirePages(box);box.querySelectorAll('[data-ce-preview]').forEach(el=>{if(resume.previews.includes(el.dataset.cePreview))el.open=true;});resume.previews=[];
