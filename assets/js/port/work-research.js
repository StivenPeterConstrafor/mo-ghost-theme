...
// A record's topics are model output, and some of them are the classifier talking to itself instead
// of naming a topic: "Marriage is not in the list — see loci_other", "Repentance placeholder",
// "Salvation—see Eternal Life". The topic vocabulary is closed, so a value is shown only when it names
...
/* TOPIC LABELS (09-13) — one rule, the same text in research-tools.js and work-research.js (a parity test
   ...
   An index row's `aliases` (typos, mirrored composites, merged duplicates; tools/mine/topic_registry.json) are keys of the same topic. */
...
function topicVocabulary(topics){if(!Array.isArray(topics)||!topics.length)return null;if(topicVocabularies.has(topics))return topicVocabularies.get(topics);const map=new Map();for(const t of topics){const name=String(t&&t.t||'');if(!name)continue;for(const v of [name,...(Array.isArray(t.aliases)?t.aliases:[])]){const k=topicKey(v);if(k&&!map.has(k))map.set(k,name);}}topicVocabularies.set(topics,map);return map;}
...
function normalize(data,topics){
 ...
 for(const [ui,u] of list(data?.units).entries()){
  ...
  const add=(kind,x,i)=>{const page=str(x.page);if(page!=='')pages.add(page);const text=kind==='summaries'?str(x.text):kind==='positions'?str(x.claim):kind==='definitions'?[x.term,x.definition].filter(Boolean).join(': '):str(x.surface||x.ref);if(!text.trim())return;const key=kind==='positions'&&x.prop_id?str(x.prop_id)+'|'+stable(x):null,prior=key&&positions.get(key);if(prior){prior.sectionTopics=cleanTopics([...prior.sectionTopics,...list(u.loci)],topics);prior.topics=cleanTopics([...prior.topics,...list(u.loci)],topics);prior.sections.push(unit);return;}let id=str(x.prop_id)||`${ui}:${kind}:${i}`;if(ids.has(id))id+=':variant:'+ui+':'+i;ids.add(id);const record={id,kind,page,text,unit,sections:[unit],sectionTopics:cleanTopics(u.loci,topics),topics:cleanTopics([...list(u.loci),...list(x.loci)],topics),raw:x};records.push(record);if(key)positions.set(key,record);};
...
function recordContext(record,books,parser,topics){return {statementTopics:cleanTopics(record.raw?.loci,topics),sectionTopics:cleanTopics(record.sectionTopics,topics),verses:record.kind==='scripture'&&parser?[parser(record.raw?.ref||record.text,books)].filter(Boolean):[]};}
...
function mount(host,opts){
 ...
 async function decorateRecords(byId){
  ...
  for(const article of results.querySelectorAll('[data-record]')){const r=byId.get(article.dataset.record);if(!r)continue;const ctx=recordContext(r,d.books,window.FRResearchData?.parseReference,d.topics),host=article.querySelector('.wr-record-context');if(!host||host.dataset.ready)continue;host.dataset.ready='1';
