...
async function compareDesk(host,state,opts={}){
  ...
  const emit=()=>{const st={...state,a:authors.map(x=>x.s)};if(opts.onState)opts.onState(st);const open=host.querySelector('.cd-open');if(open)open.href=cdURL(st);};
...
const cdViews={read(){try{const v=JSON.parse(localStorage.getItem(CD_VIEWS)||'[]');return Array.isArray(v)?v:[];}catch(_){return [];}},write(v){try{localStorage.setItem(CD_VIEWS,JSON.stringify(v));}catch(_){}}};
async function comparePage(){
  ...
  <div id="cd-host"></div>`;
  const host=$('#cd-host');
  const drawSaved=()=>{const v=cdViews.read();$('#cd-saved-count').textContent=v.length?fmtR(v.length)+' in this browser':'none yet';$('#cd-saved-list').innerHTML=v.length?v.map(x=>`<div class="cd-saved-row"><a href="${esc(x.url)}">${esc(x.name)}</a><small>${esc((x.authors||[]).join(' · '))}${x.topic?' · '+esc(x.topic):''}</small><button type="button" class="rx-text-link" data-cd-del="${esc(x.id)}">Remove</button></div>`).join(''):'<p class="rx-note">Save a comparison and it appears here. Saved views also go to your research notebook, which syncs with your account.</p>';
  ...
  drawSaved();
  $('#cd-save').onclick=async()=>{const st=cdParse(location.hash);if(!st.a.length){$('#cd-save').textContent='Add authors first';return;}
    ...
    const suggested=names.join(' · ')+(topic?' on '+topic:'');const name=prompt('Name this comparison',suggested);if(name===null)return;
    const url=cdURL(st),id='v'+Date.now().toString(36);cdViews.write([{id,name:name||suggested,url,authors:names,topic,created:new Date().toISOString()},...cdViews.read()]);drawSaved();
    try{if(window.FRResearchNotebook?.save)await FRResearchNotebook.save({type:'note',label:name||suggested,text:(name||suggested)+'\n'+url,cite:name||suggested,url,research:{kind:'comparison-view',sources:[],topics:topic?[topic]:[],authors:names}});$('#cd-save').textContent='Saved · here and in your notebook';}
    catch(_){$('#cd-save').textContent='Saved in this browser';}
    setTimeout(()=>{$('#cd-save').textContent='Save this view';},2500);};
