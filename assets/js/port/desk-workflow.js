...
(function(root,factory){const api=factory(root);if(typeof module==='object'&&module.exports)module.exports=api;else root.FRDeskWorkflow=api;})(typeof window==='undefined'?globalThis:window,function(root){
  ...
  function itemHTML(item){
    ...
    if(!quote)html='<p><strong>'+esc(['work','reading-place','reference'].includes(kind)?'Source reference':kind==='conversation'||kind==='question'?'Research question':kind==='positions'||kind==='position'?'Recorded analysis':'Research note')+'</strong></p>'+html;
  ...
  function legacyRow(key,item,kind){const at=key.lastIndexOf('|'),scope=at>=0?key.slice(0,at):'',part=/^(pld-\d+)::pldpart-([2345])$/.exec(scope),slug=item.slug||part?.[1]||scope,row=item.row||(at>=0?key.slice(at+1):''),page=item.page??(row.match(/^b(.+)-\d+$/)||[])[1],pldpart=item.pldpart||part?.[2];return {...item,slug,row,page,pldpart,kind,text:item.text||item.t||'',url:sourceURL({...item,slug,row,page,pldpart})};}
