...
async function room(slug,arg){
  ...
  if(!candidates.length){const al=await J(BLOB+'/v1/author_aliases.json').catch(()=>null);if(run!==RESEARCH_RUN)return;const to=al&&al[d.a],ts=to&&aslug(to);
...
async function compareDesk(host,state,opts={}){
  ...
  if(dl&&inp){const fill=q=>{const f=RX.fold(q||'');dl.innerHTML=rows.filter(r=>!f||RX.authorScore(r,f)>0).sort((a,b)=>RX.authorScore(b,f)-RX.authorScore(a,f)).slice(0,30).map(r=>`<option value="${esc(r.a)}">${esc(RX.shelfName(r.sh))} · ${fmtR(r.w)} works</option>`).join('');};fill('');
