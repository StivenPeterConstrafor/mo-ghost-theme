...
async function authorsIndex(){
 ...
 page.innerHTML=`<div class="research-intro"><div><h1>Authors</h1><p>Every writer in the library, shelf by shelf. Open a tradition to see its authors; open an author to read their works, follow their use of Scripture, and explore the passages behind their ideas.</p></div><a class="rx-text-link" href="/the-faith-received/constellations/">Explore citation connections</a></div>
...
async function room(slug,arg){
  ...
  <div class="ridbar">
    ...
    <div class="stats"><b>${d.n_works}</b> works · <b>${d.n_pages.toLocaleString()}</b> pages ·
      ...
      · ${esc(RX.shelves[RSH])} · <a href="/the-faith-received/constellations/#a=${encodeURIComponent(slug)}">Explore citation map</a> · <a href="${cdURL({a:[slug],sel:'',g:'work',s:'',q:''})}">Compare with other authors</a></div>
  </div>
...
async function topicPage(slug){
 ...
 page.innerHTML=`<div class="crumbs"><a id="topic-back" href="${RX.topicURL('',shelf)}">Browse ${shelf?esc(RX.shelves[shelf])+' topics':'topics'}</a></div><div class="rx-topic-heading"><h1>${esc(d.t)}</h1><a class="rx-text-link" href="/the-faith-received/constellations/#t=${encodeURIComponent(d.s||slug)}">View citation map</a></div>
...
async function compareDesk(host,state,opts={}){
  ...
  function paintMeet(){const box=host.querySelector('#cd-meet-body');if(!box||host.querySelector('#cd-dis')?.hidden||!cur)return;
    ...
    const pairHead=(A,B)=>{const ready=(A.s in recs)&&(B.s in recs);const ab=ready?citesN(recs[A.s],recs[B.s],A,B):null,ba=ready?citesN(recs[B.s],recs[A.s],B,A):null;
      ...
      return `<small>${ab===null?'counting citations…':`${esc(A.a)} cites ${esc(B.a)} ${fmtR(ab)} · ${esc(B.a)} cites ${esc(A.a)} ${fmtR(ba)}`} · ${link} · <a class="rx-text-link" href="/the-faith-received/constellations/#e=${encodeURIComponent(A.s)},${encodeURIComponent(B.s)}">in the citation web</a></small>`;};
...
async function pairPage(slugA,slugB,topicSeg){
  ...
  const block=(from,to,shard,pack,id)=>{
    ...
    return `<section class="rx-pair-block" id="${id}"><h3>${esc(from.a)} on ${esc(to.a)}<small>${fmtR(rows.length)} ${pl(rows.length,'citation')} across ${fmtR(works.length)} ${pl(works.length,'work')} · <a href="/the-faith-received/constellations/#e=${encodeURIComponent(from.s||'')},${encodeURIComponent(to.s||'')}">open in the citation web</a></small></h3>
