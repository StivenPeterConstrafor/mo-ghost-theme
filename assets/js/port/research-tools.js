...
(function(root,factory){const api=factory();if(typeof module==='object'&&module.exports)module.exports=api;else root.FRResearch=api;})(typeof globalThis!=='undefined'?globalThis:this,function(){
 ...
 function connections(topics,vocab){const pairs=new Map(),seen=new Set();for(const t of topics){for(const r of [...(t.pages||[]),...(t.pos||[])]){if(!r.w||page(r.p)===null)continue;const names=[...new Set([...(t.t&&!isRawTopic(t.t)?[t.t]:[]),...cleanTopics(r.x,vocab)])];for(let i=0;i<names.length;i++)for(let j=i+1;j<names.length;j++){const [a,b]=[names[i],names[j]].sort();const k=JSON.stringify([a,b]),pk=JSON.stringify([a,b,r.w,page(r.p)]);if(seen.has(pk))continue;seen.add(pk);if(!pairs.has(k))pairs.set(k,{a,b,pages:[]});pairs.get(k).pages.push(r);}}}return [...pairs.values()].sort((a,b)=>b.pages.length-a.pages.length||a.a.localeCompare(b.a));}
 function safeReaderURL(h){try{const u=new URL(h,'https://thefaithreceived.vercel.app');return (u.origin==='https://thefaithreceived.vercel.app'||u.origin===location.origin)&&/^\/(?:the-faith-received\/)?read(?:\.html)?\/?$/.test(u.pathname)&&u.searchParams.get('w')?'/the-faith-received/read/'+u.search+u.hash:null;}catch(_){return null;}}
