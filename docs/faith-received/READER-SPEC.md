...
**Dotted verse lists, stray ordinals and nine more sigla (owner 2026-09-16, "improve scripture detection", Voetius *Selectae disputationes* vol. 1 p. 55 — "Rom. 2. 14. 15. Psal. 19. v. 1. 8. Rom. 1. 19. 20." linked only the first verse of each):** `scriptureLocation` now continues a dot-style citation through its dotted verse list: after a verse written with a period, a period-separated number that ascends, is at most 200, is closed by punctuation or the end of the run, is not a thousands group ("40,000"), is not the ordinal of a following book ("5. Mos.") and is not followed by a smaller dotted number ("Ps. 9. 10. 18. 3." — 18 is the next psalm) is the next verse. A colon-style citation never grows a dotted tail, and a bare number before a word is prose ("14. 15 men"). `XREF_EN` gained the sigla a 150-work census showed unlinked most often: Lev, Chron, Ezek, Ioan, Iohan, Jerem, Ies (= Isaiah), Josh, Corinth. Two ordinal rules were sharpened: the dotted twin of the ordinal lookahead (`XREF_ORDNEXT_DOT`) fires only for books that cannot be cited without an ordinal (Sam., Kings, Chron., Cor., Thess., Tim., Pet., Macc., Esd., Mos., Reg., Paral.) and only inside a dot-style citation, so "Ies. 53. 1. Pet. 2." is Isaiah 53 + 1 Peter 2 while "Isa. 6. 3. John 12. 41." keeps verse 3 (John is left out: the gospel outnumbers the epistles, and "1. John 2. 3. 4. & 5." is the one known casualty); at chapter position (`XREF_ORDNEXT_CITED`) a small number followed by such a book that is itself cited is that book's ordinal ("Hebrews 1 Sam. 17:43", "Ezra 2 Chronicles 35:11", "St. John 1 John 3:4"), whereas "in Matthew 3 John said" keeps Matthew 3. A failed book match now resumes one character inside itself instead of past its trailing space, so a mis-grabbed ordinal ("5. Levit. 5.", "1. Ps. 45", "at Corinth. Romans 16") no longer hides the bare book that follows; an invalid numbered-book citation ("2 John 15:2") still stays unlinked whole. Measured against the previously deployed parser on three random 150-work samples of English lanes (450 works, 337k texts with citations): references linked +3.2 to +3.4 % (118,462→122,469; 119,944→123,882; 154,658→159,607), 912 / 1,534 / 1,857 citations extended to their full verse lists, and 20 / 27 / 22 previously linked references gone per sample — each reviewed: a wrong link removed ("the coming of Titus. 2 Cor.", "the Apostle John. 1 John 4:18", canon lists), an ordinal conversion ("John 1"+"John 4:18" → "1 John 4:18"), or the dotted-John ambiguity. Tests: `reader-scripture-links.test.cjs` (16 tests in that file, a block of 20 inputs for this change; the whole prototype test directory green); the ported reader carries the byte-identical parser block (theme PR #14 from `fix/scripture-dotted-verse-lists`, which also ports the bare-`Num.` rule the fork lacked). Measured on English lanes; the same parser runs on the Latin lanes (the live Voetius heading links in both).
