...
| **ASK_BACKEND_INSTRUCTIONS.md** | mo-workers | **Ask parity (09-11).** Why MereO's answer to the same question differs from the site's, measured marker by marker (no RRF fusion, no `MAX_PAGES`/`PER_AUTHOR` diversity, no Sentences/Summa/Scripture crosswalk, no CITE-OR-DIE prompt, `[n]` instead of `[slug/pN]`) plus the CSP `connect-src` omission that makes the ported worker unreachable; then the stage-by-stage pipeline, the constants, the model policy and its three production rules, the NDJSON dialect, the six changes MereO needs, a 12-step battery with measured expectations, and the nine source files in full | **Do this before any other Ask work**: run `ask-dev/ask.js` as the only Ask brain, add the Ask host by name to CSP `connect-src` + `mo-trusted-hosts`, render `[slug/pN]` as reader page chips, then pass the battery |
...
|---|---|---|
| 09-11 | ASK_BACKEND_INSTRUCTIONS.md — Ask parity: the exact pipeline adapted to Cloudflare, the diagnosis of the differing answers, the CSP blocker, 12-step battery, nine files verbatim | mo-workers 6f14ad4 |
...
| 09-11 | R2-SYNC-SPEC §3.1: CCEL boilerplate out of the Calvin corpus (47 works); Doronzo outlines rebuilt to the printed Ordo (11 works) | mo-ghost-theme (this commit) |
