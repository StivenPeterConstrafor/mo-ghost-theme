...
|---|---|---|
| 09-16 | READER-SPEC §6 — Scripture linker: early-modern dotted verse lists ("Rom. 2. 14. 15.") link whole; nine more sigla (Lev, Chron, Ezek, Ioan, Iohan, Jerem, Ies, Josh, Corinth); ordinal lookahead sharpened (dotted twin for ordinal-only books, chapter-position form); failed matches resume inside; +3.2–3.4 % references on 450 sampled works, every dropped old link reviewed; ported to the theme reader (byte-identical block) | mo-ghost-theme (ask-port-ui + docs branch; code PR from fix/scripture-dotted-verse-lists) |
