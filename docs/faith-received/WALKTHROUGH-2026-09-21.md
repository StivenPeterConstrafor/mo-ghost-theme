...
## Aa reader settings follow-up

Reproduced the owner's 863 × 1044 tunnel report on Tuckney: `aria-expanded=true` and the panel had dimensions, but toolbar overflow clipped it. Open toolbar popovers now escape that clipping and sit above drawers. The compact viewport rule covers tablet widths as well as phones. At 320 × 640, the bilingual settings panel scrolls within its own 422px height and ends above the 579px dock boundary. Smaller labels and wrapping button groups avoid horizontal overflow. Escape returns focus to Aa.

Verified visible, hit-testable settings at widths 320, 390, 863 and 1280; light and dark themes; text size increase/decrease; Escape dismissal; Tuckney in English and with Latin enabled; and the English-only Didache on localhost. Restored English-only and the initial dark theme after testing. Build and lint passed. Only the reader skin, Escape focus handling, and referencing asset versions changed; reader content and pairing were untouched.
