...
| mo-kit | `/preferences` | GET | JWT | Require JWT, derive email from sub; origin allowlist; per-email RL 60/15min. Reads Ghost labels + Kit tags for the /email-preferences/ page. | NEW 2026-09-09 |
| mo-kit | `/preferences` | POST | JWT | Require JWT, derive email from sub (body is 3 booleans only, no email/labels accepted); origin allowlist; per-email RL 30/15min. Writes Ghost suppression labels + Kit newsletter tags. Mailbag change 403s for non-members (server-side status check). | NEW 2026-09-09 |
| mo-kit | `/unsubscribe-all` | POST | JWT | Require JWT, derive email from sub; origin allowlist; per-email RL 5/15min. Paying member (server-re-checked) → newsletters off only. Free → Kit untag + unsubscribe, then Ghost member DELETE; every Kit step un-caught so a failure aborts BEFORE the irreversible delete; hubspot:still-billing / tier:lifetime Kit tags force the member (no-delete) path. | NEW 2026-09-09 |
| mo-kit | `/backfill-mailbag` | POST | `BACKFILL_TOKEN` bearer (or `?key=` legacy) | timingSafeEqual compare; token-gated server-to-server, above origin check like the other `/backfill-*`. One-shot pre-launch pass; re-running after launch would undo opt-outs (needs a Ghost-side exclusion first). | NEW 2026-09-09 |
