...
// ── The page's finish: rail and beta band ─────────────────────
//
// Every generated page gets the TFR rail directly under its head (the
// article header, or the feature hero), as every other TFR page has it
// (Ian, 2026-09-23: "update the document pages too"), and the beta band
// before </main>. Both were added to the committed partials by hand at
// some point (the beta band on all 89, the rail on the topics index only)
...
function finishPage(html) {
  ...
  if (!out.includes("_beta-band")) {
    ...
    if (close >= 0) {
      ...
      out = `${out.slice(0, lineStart)}  {{> "faith-received/_beta-band"}}\n</main>${out.slice(close + "</main>".length)}`;
    }
  }
  return out;
}
...
for (const file of files) {
  ...
  await writeFile(partialPath, finishPage(html.trim()) + "\n");
  ...
  if (entries && entries.length) {
    ...
    await writeFile(memPath, finishPage(memHtml.trim()) + "\n");
...
<script src="{{asset "js/faith-indexes.js"}}"></script>`;
  await writeFile(
    ...
    `{{!-- Generated by scripts/build-faith-received.mjs. Do not edit by hand. --}}\n${finishPage(topicsIndexBody.trim())}\n`
  );
